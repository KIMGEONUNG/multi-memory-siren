...
def create_mesh_with_code(
    decoder, filename, code, N=256, max_batch=64 ** 3, offset=None, scale=None
):
    start = time.time()
    ...
    while head < num_samples:
        ...
        samples[head : min(head + max_batch, num_samples), 3] = (
            decoder.forward_with_code(sample_subset, code)['model_out']
            .squeeze()#.squeeze(1)
            .detach()
            .cpu()
        )
...
def create_mesh_rgb_multi(
    ...
    for code in codes:
        ...
        rgb_values = samples[:, 4:]
        rgb_values = rgb_values.reshape(N, N, N, -1)

        end = time.time()
        print("sampling takes: %f" % (end - start))

        convert_sdfc_samples_to_ply(
            sdf_values.data.cpu(),
            rgb_values.data.cpu(),
            voxel_origin,
            voxel_size,
            ply_filename + str(code) + ".ply",
            offset,
            scale,
        )
...
def convert_sdfc_samples_to_ply(
    pytorch_3d_sdf_tensor,
    pytorch_3d_rgb_tensor,
    voxel_grid_origin,
    voxel_size,
    ply_filename_out,
    offset=None,
    ...
    numpy_3d_rgb_tensor = pytorch_3d_rgb_tensor.numpy()
