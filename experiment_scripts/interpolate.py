...
p.add_argument('--shapes',type=int,nargs=2)
########################################################

opt = p.parse_args()

# Define the model.
model = modules.SDFDecoderInference(opt.num_class,
                           opt.dim_embd,
                           opt.dim_hidden,
                           opt.num_layer)
...
interpolation_path=os.path.join(root_path,'{}_{}'.format(opt.shapes[0],opt.shapes[1]))

utils.cond_mkdir(interpolation_path)

sdf_meshing.create_mesh_interpolate(model,
                              interpolation_path,
                              shapes=opt.shapes,
                              N=opt.resolution)
